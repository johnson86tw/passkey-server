...
app.post('/api/v2/:projectId/register/verify', async c => {
	...
	const challenge = await passkeyRepo.get(['challenges', domainName, clientData.challenge])
...
app.post('/api/v2/:projectId/login/verify', async c => {
	...
	const challenge = await passkeyRepo.get<Challenge>(['challenges', domainName, clientData.challenge])
	if (!challenge) {
		return c.text('Invalid challenge', 400)
	}

	// Convert from Base64url to Uint8Array
	const credentialID = base64urlToUint8Array(credential.credentialId as string)
	const credentialPublicKey = base64urlToUint8Array(credential.publicKey as string)
...
app.post('/api/v2/:projectId/sign-initiate', async c => {
	...
	const user = await passkeyRepo.getPasskeyUserById(result.payload.userId as string)
	if (!user) return new Response('Unauthorized', { status: 401 })
	const credentials = await passkeyRepo.getCredentialsByUserId(result.payload.userId as string)
...
app.post('/api/v3/:projectId/sign-initiate', async c => {
	...
	if (!credentials) return new Response('Credentials Not Found', { status: 401 })
...
app.post('/api/v2/:projectId/sign-verify', async c => {
	...
	const { cred } = await c.req.json<{ cred: AuthenticationResponseJSON }>()
	const clientData = JSON.parse(atob(cred.response.clientDataJSON))
	const challenge = await passkeyRepo.get<string>(['challenges', domainName, clientData.challenge])
	if (!challenge) return c.text('Invalid challenge', 400)

	const user = await passkeyRepo.getPasskeyUserById(cred.response.userHandle as string)
	...
	if (verification.verified) {
		...
		const publicKeyBase64 = btoa(String.fromCharCode(...new Uint8Array(base64urlToUint8Array(publicKey))))
